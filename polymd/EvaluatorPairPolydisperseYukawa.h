...
#ifndef __PAIR_EVALUATOR_POLYDISPERSE_YUKAWA_H__
#define __PAIR_EVALUATOR_POLYDISPERSE_YUKAWA_H__
...
/*! \file EvaluatorPairPolydisperseYUKAWA.h
...
/*! <b>General Overview</b>
    ...
    EvaluatorPairPolydisperseYUKAWA is a low level computation class that computes the YUKAWA pair potential V(r). As the standard
    ...
    EvaluatorPairPolydisperseYUKAWA evaluates the function:
    \f[ V_{\mathrm{YUKAWA}}(r) = 4 \varepsilon \left[ \left( \frac{\sigma}{r} \right)^{12} -
                                            \alpha \left( \frac{\sigma}{r} \right)^{6} \right] \f]
    broken up as follows for efficiency
    \f[ V_{\mathrm{YUKAWA}}(r) = r^{-6} \cdot \left( 4 \varepsilon \sigma^{12} \cdot r^{-6} -
                                            4 \alpha \varepsilon \sigma^{6} \right) \f]
    . Similarly,
    \f[ -\frac{1}{r} \frac{\partial V_{\mathrm{YUKAWA}}}{\partial r} = r^{-2} \cdot r^{-6} \cdot
            \left( 12 \cdot 4 \varepsilon \sigma^{12} \cdot r^{-6} - 6 \cdot 4 \alpha \varepsilon \sigma^{6} \right) \f]

    The YUKAWA potential does not need diameter or charge. Two parameters are specified and stored in a Scalar2. \a lj1 is
...
*/
class EvaluatorPairPolydisperseYukawa
    {
    public:
        ...
        */
        DEVICE EvaluatorPairPolydisperseYukawa(Scalar _rsq, Scalar _rcutsq, const param_type& _params)
            : rsq(_rsq), rcutsq(_rcutsq), v0(_params.x), eps(_params.y), scaledr_cut(_params.z), kappa(_params.w), c0(-0.0081758995207425201542184307608647), c1(0.0014969152162365760170546672671884), c2(-0.000070465474555665792151428357361285)
            {
                ...
                c1 = (expfactor*(Scalar(5) + Scalar(5)*kappa*scaledr_cut + kappa*kappa*scaledr_cut*scaledr_cut)*v0)/(Scalar(4)*scaledr_cut*scaledr_cut*scaledr_cut);
                c2 = -((expfactor*(Scalar(3) + Scalar(3)*kappa*scaledr_cut + kappa*kappa*scaledr_cut*scaledr_cut)*v0)/(Scalar(8)*scaledr_cut*scaledr_cut*scaledr_cut*scaledr_cut*scaledr_cut));
            }
        ...
        DEVICE bool evalForceAndEnergy(Scalar& force_divr, Scalar& pair_eng, bool energy_shift)
            {
                ...
                if (rsq < actualcutsq && v0 != 0)
                    ...
                    pair_eng = v0*expfactor*rinv+c0+c1*_rsq + c2*_rsq*_rsq;
        ...
        */
        static std::string getName()
            {
            return std::string("polydisperse_yukawa");
            }
...
#endif // __PAIR_EVALUATOR_POLYDISPERSE_YUKAWA_H__
