...
#ifndef __PAIR_EVALUATOR_FORCE_SHIFTED_LJ_PLUGIN_H__
#define __PAIR_EVALUATOR_FORCE_SHIFTED_LJ_PLUGIN_H__
...
/*! <b>General Overview</b>
    ...
    EvaluatorPairLJPlugin is a low level computation class that computes the LJ pair potential V(r). As the standard
    ...
    EvaluatorPairLJPlugin evaluates the function:
...
*/
class EvaluatorPairForceShiftedLJPlugin
    {
    public:
        ...
        */
        DEVICE EvaluatorPairForceShiftedLJPlugin(Scalar _rsq, Scalar _rcutsq, const param_type& _params)
        ...
        */
        DEVICE bool evalForceAndEnergy(Scalar& force_divr, Scalar& pair_eng, bool energy_shift)
            {
            Scalar sigma = 0.5*(d_i+d_j);
            Scalar actualcutsq = rcutsq*sigma*sigma;
            // compute the force divided by r in force_divr
            //if (rsq < rcutsq && lj1 != 0)
            if (rsq < actualcutsq && lj1 != 0)
                {
                ...
                Scalar rcut_r_inv = fast::rsqrt(rsq*actualcutsq);
        ...
        */
        static std::string getName()
            {
            return std::string("force_shifted_lj_plugin");
            }
        #endif
    ...
    protected:
        ...
        Scalar d_i;     //!< lj2 parameter extracted from the params passed to the constructor
        Scalar d_j;     //!< lj2 parameter extracted from the params passed to the constructor
    };

#endif // __PAIR_EVALUATOR_FORCE_SHIFTED_LJ_PLUGIN_H__
